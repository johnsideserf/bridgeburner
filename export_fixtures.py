...
def write_all(out_dir, seeds=(1, 2, 3)):
    ...
    # ES-module copy for the browser (no JSON import assertions needed).
    web = os.path.join(os.path.dirname(out_dir.rstrip("/")), "..") if out_dir.endswith("fixtures") else out_dir
    with open(os.path.join(web, "strategies.js"), "w") as f:
        f.write("// Generated by export_fixtures.py from solve.SEEDS / solve.RULESETS. Do not edit.\n")
        f.write("export const GENE_KEYS = " + json.dumps(GENE_KEYS) + ";\n")
        f.write("export const STRATEGIES = " + json.dumps({k: list(v) for k, v in SEEDS.items()}) + ";\n")
        f.write("export const RULESETS = " + json.dumps(RULESETS) + ";\n")
        f.write("export const CURRENT_RULES = " + json.dumps(CURRENT_RULES) + ";\n")
    paths = []
